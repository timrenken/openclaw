...
- During reconnect, an attempted message stays in the transcript with a **Waiting for reconnect** status. Messages that have not been attempted stay in the composer queue. Accepted inputs appear once even while the browser retains their retry payload. Reordering the remaining queue preserves the position of messages already handed to the Gateway. **Discard** removes only the local pending copy; it does not cancel a message the Gateway already received.
...
- When a reply attachment cannot be read or prepared, WebChat preserves any deliverable attachments. It shows one short failure warning without exposing local filesystem paths.
